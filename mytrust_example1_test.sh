...
# we use instead docker and run it in the notebook
#./run-trust4 -f hg38_bcrtcr.fa --ref human_IMGT+C.fa -b example/example.bam -o example_test

if [ ! -e "example/Output/TRUST_example_1_report.tsv" ]; then
...
if [[ $(diff <(sort example/Output/TRUST_example_1_report.tsv) <(sort example/TRUST_example_report.tsv) | wc -l) -ge 1 ]]; then
