...
if [ ! -e "example/Output/TRUST_example_report.tsv" ]; then
...
if [[ $(diff <(sort example/Output/TRUST_example_report.tsv) <(sort example/TRUST_example_report.tsv) | wc -l) -ge 1 ]]; then
